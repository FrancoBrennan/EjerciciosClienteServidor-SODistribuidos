...
Pagina *paginas; // Array de páginas en memoria compartida
...
void init_memoria_semaforos()
{
    int shmid = shmget(IPC_PRIVATE, MAX_PAG * sizeof(Pagina), IPC_CREAT | 0666);
    if (shmid < 0)
    {
        perror("shmget");
        exit(1);
    ...
    paginas = (Pagina *)shmat(shmid, NULL, 0);
    if (paginas == (Pagina *)-1)
    {
        perror("shmat");
        exit(1);
    ...
    for (int i = 0; i < MAX_PAG; i++)
    {
        ...
        sem_init(&paginas[i].semaforo, 1, 1); // Uso de semáforos en memoria compartida
    }
}
